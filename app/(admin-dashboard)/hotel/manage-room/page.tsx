...
const Page = () => {
  ...
  useEffect(() => {
    const fetchRooms = async () => {
      try {
        const response = await fetch("https://yrpitsolutions.com/tourism_api/api/admin/hotel_rooms");
        if (!response.ok) {
 ...
 const handleSubmit = async (e: React.FormEvent) => {
  ...
  for (const key in formData) {
    if (key === 'featured_images') {
      ...
      formDataToSend.append(key, formData[key as keyof HotelFormData]);
    }
  }
  ...
  try {
    const response = await fetch("https://yrpitsolutions.com/tourism_api/api/admin/hotel_rooms", {
      ...
      headers: {
        Authorization: `Bearer ${token}`,
        // Note: Don't include Content-Type for FormData
      },
      body: formDataToSend,
    });
  ...
  <div className="mt-[20px]">
    <button type="submit" onClick={handleSubmit} className="btn-primary font-semibold">
      Add New
    </button>
  </div>
        ...
        <div className="col-span-12 lg:col-span-6 p-4 md:p-6 lg:p-10 rounded-2xl bg-white">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <input
              ...
              onChange={handleInputChange}
            />
            <p className="mt-6 mb-4 text-xl font-medium">AMENITIES :</p>
            <input
              type="text"
              name="amenities"
              className="w-full border py-2 px-3 lg:px-4 focus:outline-none rounded-md text-base"
        ...
        <div className="col-span-12 lg:col-span-6 p-4 md:p-6 lg:p-10 rounded-2xl bg-white">
          ...
          <div className="overflow-x-auto">
            <table className="w-full whitespace-nowrap">
              ...
              <tbody>
                {rooms.length > 0 ? rooms.map(({ id, room_name, room_price, status }) => (
                  <tr
                    key={id}
                    className="border-b border-dashed hover:bg-[var(--bg-1)] duration-300">
                    <td className="py-3 lg:py-4 px-2">{room_name}</td>
                    <td className="py-3 lg:py-4 px-2">{room_price}</td>
                    <td className="py-3 lg:py-4 px-2">{status === "1" ? "Active" : "Inactive"}</td>
                  ...
                  <tr>
                    <td colSpan={4} className="text-center py-3">No rooms available</td>
                  </tr>
                )}
              </tbody>
            </table>
