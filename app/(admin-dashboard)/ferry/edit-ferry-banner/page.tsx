...
const AddNewBanner = () => {
  ...
  useEffect(() => {
    if (bannerId) {
      axios
        .get(`https://yrpitsolutions.com/tourism_api/api/get_ferry_home_banner_by_id/${bannerId}`)
        .then((response) => {
          const data = response.data;
          // Prefill with existing banner data
          setDesktopBannerPreview(data.data.ferry_desktop_banner);
          setMobileBannerPreview(data.data.ferry_mobile_banner);
  ...
  const handleSubmit = async () => {
    ...
    try {
      await axios.post(
        `https://yrpitsolutions.com/tourism_api/api/admin/update_ferry_home_banner_by_id/${bannerId}`,
        formData,
        {
          headers: {
