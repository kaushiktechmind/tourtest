...
const Page = () => {
  ...
  const handleAddFAQ = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch(
        "https://yrpitsolutions.com/tourism_api/api/admin/save_activity_faq",
        {
          method: "POST",
          headers: {
  ...
  const handleDeleteFAQ = async (id: number) => {
    ...
    try {
      const response = await fetch(
        `https://yrpitsolutions.com/tourism_api/api/admin/delete_activity_faq_by_id/${id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${accessToken}`,
  ...
  return (
    <div className="bg-[var(--bg-2)]">
      <div className="flex items-center justify-between flex-wrap px-3 py-5 md:p-[30px] gap-5 lg:p-[60px] bg-[var(--dark)]">
        <h2 className="h2 text-white">Activity FAQs</h2>
                ...
                {currentFAQs.map(({ id, faq_title, created_at }) => (
                  ...
                  >
                    ...
                    <td className="py-3 lg:py-4 px-2 flex gap-2 items-center">
                      <Link
                        href={`/activity/edit-activity-faq?faqId=${id}`}
