...
interface Attribute {
  id: number;
  cab_attribute_name: string;
  cab_attribute_logo: string;
}
...
const Page = () => {
  ...
  useEffect(() => {
    if (searchQuery === "") {
      ...
      const filtered = attributes.filter((attribute) =>
        attribute.cab_attribute_name.toLowerCase().includes(searchQuery.toLowerCase())
      );
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(
        "https://yrpitsolutions.com/tourism_api/api/admin/save_cab_attribute",
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        ...
        setAttributes((prev) => [
          ...
          {
            id: result.data.id,
            cab_attribute_name: name,
            cab_attribute_logo: result.data.cab_attribute_logo,
          },
        ]);
        // Reset the form
        setName("");
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(
        `https://yrpitsolutions.com/tourism_api/api/admin/delete_cab_attribute_by_id/${attributeToDelete}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${accessToken}`,
  ...
  return (
    <div className="bg-[var(--bg-2)]">
      <div className="flex items-center justify-between flex-wrap px-3 py-5 md:p-[30px] gap-5 lg:p-[60px] bg-[var(--dark)]">
        <h2 className="h2 text-white">Cab Attributes</h2>
        <Link href="/cab/all-cab" className="btn-primary">
          <EyeIcon className="w-5 h-5" /> View All Cab
        </Link>
      </div>
                ...
                {loading ? (
                  ...
                  currentItems.map((attribute, index) => (
                    <tr key={attribute.id} className="border-b border-dashed">
                      ...
                      <td className="py-3 lg:py-4 px-2">{attribute.cab_attribute_name}</td>
                      <td className="py-3 lg:py-4 px-2">
                        <Image
                          src={attribute.cab_attribute_logo}
                          alt={attribute.cab_attribute_name}
                          width={40}
                          height={40}
                      ...
                      <td className="py-3 lg:py-4 px-2 flex items-center space-x-2">
                        <Link href={`/cab/edit-cab-attributes?attributeId=${attribute.id}`}>
