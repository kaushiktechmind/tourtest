...
const Page = () => {
  ...
  useEffect(() => {
    const filtered = faqs.filter((faq) =>
      faq.cab_faq_title.toLowerCase().includes(searchQuery.toLowerCase())
    );
  ...
  const handleDeleteFAQ = async (id: number) => {
    ...
    try {
      const response = await fetch(
        `https://yrpitsolutions.com/tourism_api/api/admin/delete_cab_faq_by_id/${id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${accessToken}`,
  ...
  return (
    <div className="bg-[var(--bg-2)]">
      <div className="flex items-center justify-between flex-wrap px-3 py-5 md:p-[30px] gap-5 lg:p-[60px] bg-[var(--dark)]">
        <h2 className="h2 text-white">Cab FAQs</h2>
        <Link href="/all-cabs" className="btn-primary">
