...
const Page = () => {
  ...
  const handleSearch = () => {
    ...
    const filtered = includes.filter((include) =>
      include.cab_inclusion_title.toLowerCase().includes(lowercasedSearch)
    );
  ...
  return (
    <div className="bg-[var(--bg-2)]">
      <div className="flex items-center justify-between flex-wrap px-3 py-5 md:p-[30px] gap-5 lg:p-[60px] bg-[var(--dark)]">
        <h2 className="h2 text-white">Cab Includes</h2>
      </div>
                  ...
                  {loading ? (
                    ...
                    currentItems.map((include) => (
                      <tr key={include.id} className="border-b hover:bg-[var(--bg-1)]">
                        <td className="py-3 px-2">{include.cab_inclusion_title}</td>
                        <td className="py-3 px-2 flex gap-2">
                          <Link href={`/cab/edit-cab-include?includeId=${include.id}`} className="text-primary">
