...
const Page = () => {
      ...
      const fetchData = async () => {
        ...
        try {
          const response = await fetch(
            `https://yrpitsolutions.com/tourism_api/api/admin/get_package_attribute_by_id/${attributeId}`,
            {
              method: "GET",
              headers: {
          ...
          if (data && data.data) {
            setAmenityData({
              package_attribute_name: data.data.package_attribute_name || "",
              package_attribute_logo: data.data.package_attribute_logo || "",
            });
          }
        } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!amenityData.package_attribute_name.trim()) {
    ...
    form.append("package_attribute_name", amenityData.package_attribute_name);

    // Send existing amenity logo if no new file is uploaded
    if (iconFile) {
      form.append("package_attribute_logo", iconFile);
    } else {
      // If no new logo is uploaded, send the existing package_attribute_logo URL
      form.append("package_attribute_logo", amenityData.package_attribute_logo);
    }
    ...
    try {
      const response = await fetch(
        `https://yrpitsolutions.com/tourism_api/api/admin/update_package_attribute_by_id/${attributeId}`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      router.push("/package/package-attribute");
    } catch (error) {
  ...
  return (
    <div className="bg-[var(--bg-2)]">
      <div className="flex items-center justify-between flex-wrap px-3 py-5 md:p-[30px] gap-5 lg:p-[60px] bg-[var(--dark)]">
        ...
        <Link href="/package/package-attribute" className="btn-primary">
        ...
        <div className="col-span-12 flex justify-center">
          <div className="lg:w-6/12 p-4 md:p-6 lg:p-10 rounded-2xl bg-white">
            ...
            <form onSubmit={handleSubmit}>
              <label
                htmlFor="package_attribute_name"
              ...
              <input
                type="text"
                id="package_attribute_name"
                value={amenityData.package_attribute_name}
              ...
              {amenityData.package_attribute_logo && (
                <div className="mt-4">
                  <Image
                    width={22}
                    height={22}
                    src={iconFile ? iconImage : amenityData.package_attribute_logo}
