...
const EditPackageFAQ = () => {
      ...
      if (faqId) {
        try {
          const response = await fetch(
            `https://yrpitsolutions.com/tourism_api/api/admin/get_package_faq_by_id/${faqId}`
          );
          const data = await response.json();

          // Set the values for title and description
          setFaqTitle(data.package_faq_title);
          setDescription(data.package_faq_description);
          console.log("Description from API:", data.package_faq_description);
        } catch (error) {
  ...
  const handleUpdateFaq = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://yrpitsolutions.com/tourism_api/api/admin/update_package_faq_by_id/${faqId}`,
        {
      ...
      if (response.ok) {
        ...
        router.push("/package/package-faq");
      } else {
