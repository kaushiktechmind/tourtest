...
const Page = () => {
  ...
  useEffect(() => {
    const filtered = faqs.filter((faq) =>
      faq.package_faq_title.toLowerCase().includes(searchQuery.toLowerCase())
    );
  ...
  const handleAddFAQ = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch(
        "https://yrpitsolutions.com/tourism_api/api/admin/save_package_faq",
        {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({
            package_faq_title: faqTitle,
            package_faq_description: plainTextDescription,
          }),
        }
      );
      ...
      if (result.data) {
        const newFaq: Faq = {
          id: result.data.id,
          package_faq_title: result.data.package_faq_title,
  ...
  const handleDeleteFAQ = async (id: number) => {
    ...
    try {
      const response = await fetch(
        `https://yrpitsolutions.com/tourism_api/api/admin/delete_package_faq_by_id/${id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${accessToken}`,
  ...
  return (
    <div className="bg-[var(--bg-2)]">
      <div className="flex items-center justify-between flex-wrap px-3 py-5 md:p-[30px] gap-5 lg:p-[60px] bg-[var(--dark)]">
        <h2 className="h2 text-white">Package FAQs</h2>
        ...
        <div className="col-span-12 lg:col-span-6 p-4 md:p-6 lg:p-10 rounded-2xl bg-white">
          ...
          <div className="overflow-x-auto">
            <table className="w-full whitespace-nowrap">
              ...
              <tbody>
                {currentFAQs.map(({ id, package_faq_title, created_at }) => (
                  <tr
                    key={id}
                  ...
                  >
                    ...
                    <td className="py-3 lg:py-4 px-2 flex gap-2 items-center">
                      <Link
                        href={`/package/edit-package-faq?faqId=${id}`}
