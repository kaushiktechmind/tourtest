...
import { allBoats } from "@/public/data/allboats";
...
const Page = () => {
  return (
    <div className="bg-[var(--bg-2)]">
      <div className="flex items-center justify-between flex-wrap px-3 py-5 md:p-[30px] gap-5 lg:p-[60px] bg-[var(--dark)]">
        <h2 className="h2 text-white">All Boat</h2>
        <Link href="/boat/add-new-boat" className="btn-primary">
          <PlusCircleIcon className="w-5 h-5" /> Add New Boat
        </Link>
      </div>
            ...
            <table className="w-full whitespace-nowrap">
              <thead>
                <tr className="text-left bg-[var(--bg-1)] border-b border-dashed">
                  <th className="py-3 lg:py-4 px-2 lg:px-4">Boat Name</th>
              ...
              <tbody>
                {allBoats.map(
                  ({ id, Agents, Date, location, name, rating, status }) => (
                    <tr
                      key={id}
 ...
 lg:px-4">
                      ...
                      <td className="py-3 lg:py-4 px-2">{Agents}</td>
