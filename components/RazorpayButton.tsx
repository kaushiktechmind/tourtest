...
const RazorpayButton: React.FC<RazorpayButtonProps> = ({
  ...
  const noOfNights = localStorage.getItem("noOfNights");
  ...
  //         const roomsMinusURL = `https://yrpitsolutions.com/tourism_api/api/rooms_minus/${hotelId}/${location}/${roomId}/${startDate}/${endDate}/${totalRooms}`;
  //         await axios.get(roomsMinusURL, {
  //           headers: {
  //             Authorization: `Bearer ${accessToken}`,
  //           },
  //         });
 ...
 <div class="container-wrapper">
	<div class="container">
     ...
     <div style="border-left: 1px solid #b6b6b6; height: 200px;"></div>
            ...
            <p>Hotel: ${hotelDetails?.hotel_name}</p>
        <p>Room Type: ${roomName}</p>
        <p>No of Rooms: ${totalRooms}</p>
				  <p>Booking ID: ${bookingID}</p>
          <p>Booking Date: ${todayDate}</p>
				<p>Check In Date: ${formattedStartDate}</p>
        <p>Check Out Date: ${formattedEndDate}</p>
    ...
    <br>
		...
		<div class="main-header">
            ...
            <td>${adults}</td>
            <td>₹${roomPrice}</td>
            <td>₹${storedAdultPrice}</td>
				</tr>
				<tr>
					  <td>Child (6-11)</td>
            <td>${children}</td>
          <td>
  ${isNaN(Number(storedChildPrice) / Number(children)) || (Number(storedChildPrice) / Number(children)) === 0
              ? '0'
              : `₹${Number(storedChildPrice) / Number(children)}`}
</td>
<td>
  ${isNaN(Number(storedChildPrice)) || Number(storedChildPrice) === 0
              ? '0'
              : `₹${Number(storedChildPrice)}`}
</td>
        ...
        <tr>
         ...
         <tr>
					...
					<td>${extraBeds}</td>
				<td>
  ${isNaN(Number(storedExtraBedPrice) / Number(extraBeds)) || (Number(storedExtraBedPrice) / Number(extraBeds)) === 0
              ? '0'
              : `₹${Number(storedExtraBedPrice) / Number(extraBeds)}`}
</td>
<td>
  ${isNaN(Number(storedExtraBedPrice)) || Number(storedExtraBedPrice) === 0
              ? '0'
              : `₹${Number(storedExtraBedPrice)}`}
</td>
				...
				<tr>
					...
					<td><strong>₹${storedTotalPrice}</strong></td>
				</tr>
       <tr>
					<td colspan="3" align="right">No. of nights</td>
					<td><strong>${noOfNights}</strong></td>
				</tr>
				<tr>
          ...
          const pdf = html2pdf()
            ...
            .then(async (pdf: { output: (arg0: string) => any }) => {
              ...
              formData.append(
                "invoice_pdf",
                pdfBlob,
                `${response.razorpay_payment_id}.pdf`
              ); // Attach the PDF
              ...
              formData.append("hotel_name", String(hotelDetails?.hotel_name));
              formData.append("payment_method", "Razorpay");
              formData.append("address", address || "");
              formData.append("passport_no", passport || "");
              formData.append("country", country || "");
              formData.append("starting_date", String(startDate)); // Replace with correct value
              formData.append(
                "service_type",
                String(hotelDetails?.hotel_or_home_stay)
              ); // Adjust if needed
              formData.append("adult", adults || "");
              formData.append("childs", children || "");
              formData.append("infants", infants || "");
              formData.append("no_of_rooms", totalRooms || "");
              formData.append("child_price", storedChildPrice || "");
              formData.append("extra_bed_price", storedExtraBedPrice || "");
              formData.append("extra_bed_count", extraBeds || "");
