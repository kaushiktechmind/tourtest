...
<td>
  ${isNaN(Number(storedExtraBedPrice)) || Number(storedExtraBedPrice) === 0
       ...
       <tr>
                  ...
                  <tr>
                     ...
                     <td>
                        <strong>
                        ₹${Number(discountedPrice) > 0 ? discountedPrice : grandTotal / 100}/-
                        </strong>
                     </td>
                  </tr>
			</table>
         ...
         `;
          ...
          const pdf = html2pdf()
            ...
            .then(async (pdf: { output: (arg0: string) => any }) => {
              ...
              formData.append("invoice_pdf", pdfBlob, `${response.razorpay_payment_id}.pdf`);
              ...
              formData.append("starting_date", String(startDate));
              formData.append("service_type", String(hotelDetails?.hotel_or_home_stay));
              ...
              if (storePaymentResponse.status === 200) {
                ...
                // Fetch the invoice and open SweetAlert
                await fetchAndStoreInvoiceUrl(response.razorpay_payment_id, bookingID);
              } else {
          ...
          setLoading(false);
          router.replace(`/receipt?payment_id=${response.razorpay_payment_id}&amount=${data.amount / 100}&hotelId=${hotelId}&roomId=${roomId}`);
        } catch (error) {
  ...
  const fetchAndStoreInvoiceUrl = async (paymentId: string, bookingId: string) => {
    const customerId = localStorage.getItem("id");
    ...
    try {
      ...
      if (invoiceUrl) {
        openSweetAlert(invoiceUrl, paymentId, bookingId);
      } else {
