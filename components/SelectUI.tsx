...
export default function SelectUI({ options }: OptionTypes) {
  const [selected, setSelected] = useState<OptionType | null>(null); // Start with null

  // Use effect to set the first option as default after options load
  useEffect(() => {
    if (options.length > 0) {
      ...
      <Listbox value={selected} onChange={setSelected}>
        <div className="relative">
          <Listbox.Button className="relative w-full cursor-default py-3 px-3 lg:px-6 flex gap-3 items-center sm:text-sm bg-white border rounded-[5px]">
            <span className="block">{selected ? selected.name : "Select an option"}</span>
