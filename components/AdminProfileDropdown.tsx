...
export default function AdminProfileDropdown() {
  const [name, setName] = useState("Guest User1");
  ...
  const handleLogout = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        localStorage.removeItem("email");// Clear the username
        localStorage.removeItem("access_token"); // Clear the access token
        // Redirect or perform other actions after logout
        window.location.href = "/auth/signin"; // Example: redirect to the login page
      } else {
